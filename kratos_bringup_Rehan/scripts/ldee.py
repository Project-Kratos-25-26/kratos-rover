...
class Drive(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Joy, "/joy0", self.callback, 10)
        ...
        self.publisher = self.create_publisher(
            Float32MultiArray, "/ld_filler_topic", 10
        )
    ...
    def teleop(self, linear, rotational, speed):
        self.rover_x = (
            linear * ((speed + 1) / 2) + rotational * ((speed + 1) / 2)
        ) * 100
        self.rover_z = (
            linear * ((speed + 1) / 2) - rotational * ((speed + 1) / 2)
        ) * 100
    ...
    def move(self):
        ...
        if velx >= 0 and velz >= 0:
            ...
            right_wheel_front = ((velx + velz) / 2 + (velx - velz) / 2) * (
                100.0 / 148.0
            )
